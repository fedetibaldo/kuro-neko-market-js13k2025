...
class TrophyCase {
	...
	static async notifyTrophy(trophy) {
		...
		const tempText = new GameText({
			...
			size: Game.viewRes
				.diff(padding.mul(2))
				.diff(implicitButtonPadding.mul(2)),
		});
		const startPos = new Vector(
			padding.x,
			-tempText.getTextMetrics().y - implicitButtonPadding.mul(2).y
		);
		const popup = new Button({
			text: trophy.name,
			freezed: true,
			size: new Vector(
				Game.viewRes.diff(padding.mul(2)).x,
				tempText.getTextMetrics().y + implicitButtonPadding.mul(2).y
			),
			pos: startPos,
		});
		...
		await Promise.all([
			Animate.slide(popup, { delay: 1000, duration: 400, to: startPos })
				.promise,
...
TrophyCase.trophies = [
	...
	new Trophy(
		"RETURNING CUSTOMER",
		"OPEN THE GAME TEN TIMES",
		() => +localStorage.getItem("logins") >= 10
	),
	new Trophy(
		"TIME TO SETTLE IT DOWN",
		"UNLOCK 'GET SQUARE'",
		() => TrophyCase.getTrophy("3 IN A ROW").getBest() >= 20
	),
	new Trophy(
		"YOU'RE AWESOME",
		"UNLOCK 'HIGH FIVE'",
		() => TrophyCase.getTrophy("GET SQUARE").getBest() >= 15
	),
	new Trophy(
		"THEORETICALLY ENDLESS FUN",
		"UNLOCK 'ARCADE'",
		() => TrophyCase.getTrophy("HIGH FIVE").getBest() >= 10
	),
	new Trophy(
		"CULTURIST",
		"UNLOCK 'SIX PACK'",
		() => TrophyCase.getTrophy("ARCADE").getBest() >= 30
	),
	new Trophy(
		"OVERACHIEVER I",
		"SCORE AT LEAST 25 POINTS IN '3 IN A ROW'",
		() => TrophyCase.getTrophy("3 IN A ROW").getBest() >= 25
	),
	new Trophy(
		"OVERACHIEVER II",
		"SCORE AT LEAST 20 POINTS IN 'GET SQUARE'",
		() => TrophyCase.getTrophy("GET SQUARE").getBest() >= 20
	),
	new Trophy(
		"OVERACHIEVER III",
		"SCORE AT LEAST 15 POINTS IN 'HIGH FIVE'",
		() => TrophyCase.getTrophy("HIGH FIVE").getBest() >= 15
	),
	new Trophy(
		"OVERACHIEVER IV",
		"SCORE AT LEAST 40 POINTS IN 'ARCADE'",
		() => TrophyCase.getTrophy("ARCADE").getBest() >= 40
	),
	new Trophy(
		"OVERACHIEVER V",
		"SCORE AT LEAST 15 POINTS IN 'SIX PACK'",
		() => TrophyCase.getTrophy("SIX PACK").getBest() >= 15
	),
	new Trophy("ALL STARS", "UNLOCK ALL TROPHIES", () =>
		TrophyCase.trophies
			.filter(
				(trophy) => !(trophy instanceof Record) && trophy.name != "ALL STARS"
			)
			.every((trophy) => trophy.unlocked)
	),
];
...
class GameText extends GameObject {
	...
	render(ctx) {
		...
		if (this.get("align") == "center") {
			startPos = this.size
				.mul(1 / 2)
				.diff(this.getTextMetrics(this.get("text")).mul(1 / 2));
		}
				...
				.forEach((char) => {
					...
					if (!this.color.equals(Color.white)) {
						...
						for (let i = 0; i < imageData.data.length; i += 4) {
							...
							imageData.data[i + 1] =
								(this.color.g / 255) * imageData.data[i + 1];
							imageData.data[i + 2] =
								(this.color.b / 255) * imageData.data[i + 2];
						}
				...
				if (cumulativeWidth + width > this.size.x) {
					if (width > this.size.x) {
						...
						const index = word.split(``).findIndex((char, index, word) => {
							...
							return (
								cumulativeWidth +
									this.getTextMetrics(wordSoFar.join(``)).x +
									hyphenWidth >
								this.size.x
							);
						});
	...
	getTextMetrics(text) {
		const lines =
			this.fixedSize && !text ? this.splitText(this.get("text")) : [text];
		return new Vector(
			Math.max(
				...lines.map((line) =>
					line.split(``).reduce((width, char) => {
						return width + this.font.getWidth(char) * this.fontSize;
					}, 0)
				)
			),
			lines.length * this.font.getHeight() * this.fontSize * this.lineHeight -
				(this.lineHeight - 1)
		).floor();
	}
}
...
class GameFont {
	...
	async load() {
		...
		for (let range of this.meta.ranges) {
			...
			const [lowerBound, upperBound] = range.map((charCode) =>
...
class Rectangle extends GameObject {
	constructor({ color = new Color(), size = new Vector(), ...options }) {
		super(options);
		this.color = color;
		this.size = size;
	}
...
class ResultsScreen extends GameObject {
	constructor({ score = 0, level, ...options }) {
	...
	async entryAnimation() {
		...
		await Animate.slide(title, { duration: 600, delay: 400, to: ogPos })
			.promise;
		const scoreLabel = this.getChild("scoreLabel");
		const score = this.getChild("score");
		const scoreAnimation = Animate.counter(score, {
			duration: 1200,
			to: this.score,
		});
		await Promise.all([
	...
	retry() {
		...
		if (this.level instanceof Arcade) {
			Game.root
				.getChild("main")
				.addChild(new Arcade({ ...this.level, children: null, opacity: 1 }));
		} else {
			Game.root
				.getChild("main")
				.addChild(new Level({ ...this.level, children: null, opacity: 1 }));
		}
		this.destroy();
	}
	async goToMenu() {
		...
		const nextScreen =
			(this.level instanceof Arcade &&
				!(this.previousBest < 30 && this.score >= 30)) ||
			(this.level.name == "HIGH FIVE" &&
				this.previousBest < 10 &&
				this.score >= 10)
				? new Menu(config)
		...
		await Promise.all([
			Animate.slide(this, {
				...
				to: this.pos.add(viewWidth),
			}).promise,
			Animate.fadeOut(this, { duration: slideDuration }).promise,
			Animate.slide(nextScreen, { duration: slideDuration, to: new Vector() })
				.promise,
...
class Sprite extends GameObject {
	constructor({ asset = ``, spritesheet = assets, ...options }) {
		super({ asset, spritesheet, ...options });
	}
	render(ctx) {
		ctx.drawImage(
			this.spritesheet.source,
			this.spritesheet.getOffset(this.asset),
			0,
...
class ColoredSprite extends GameObject {
	constructor({
		...
		this.colorArray = this.toColorArray(
	...
	update(deltaT) {
		...
		if (targetColor) {
			const newColor = this.currentColor.shift(
				targetColor,
				(deltaT / this.shiftDur) * 255
			);
			this.colorArray = this.colorArray.map((color) =>
				color.a == 255 ? newColor : color
			);
	...
	toImageData(colorArray) {
		return new ImageData(
			Uint8ClampedArray.from(
				colorArray.reduce(
					(data, color) => [...data, color.r, color.g, color.b, color.a],
					[]
				)
			),
			this.canvas.width
...
class Tile extends Area {
	...
	update(deltaT) {
		...
		if (targetColor) {
			this.currentColor = this.currentColor.shift(
				targetColor,
				(deltaT / this.shiftDur) * 255
			);
		}
	...
	createChildren() {
		const children = [
			new GameText({
				...
				size: this.size.diff(
					new Vector(
						this.padding + this.border + (this.locked ? 11 / 2 : 0),
	...
	highlight() {
		this.children[0] &&
			this.children[0].children.forEach(
				(child) => (child.accentColor = Color.yellow)
			);
	}
	swap() {
		...
		if (oldFlex) {
			Promise.all(
				oldFlex.children.map((oldChild, index) => {
...
class Countdown extends GameObject {
	...
	update(deltaT) {
		if (!this.completed) {
			this.progress = Math.max(
				0,
				Math.min(this.progress + deltaT, this.duration)
			);
			this.trailColor.a = Math.max(
				this.trailColor.a - (deltaT / this.trailFade) * 255,
				0
			);
			if (this.progress == this.duration) {
				this.completed = true;
	...
	render(ctx) {
		...
		const progressLength =
			this.length - (this.length / this.duration) * this.progress;

		const startProgress = Math.round((this.length - progressLength) / 2);
		const endProgress = Math.round(
			progressLength + (this.length - progressLength) / 2
		);
		...
		const trailLength =
			this.length - (this.length / this.duration) * this.trail;
...
class GameBoard extends GameObject {
	constructor({ data = "0", combo = [], ...options }) {
		...
		return this.grid.reduce(
			(children, row, rowIndex) => [
				...
				...row.map((col, colIndex) =>
					this._createChild(new Vector(colIndex, rowIndex), col)
				),
			],
			[]
		);
	...
	replaceTiles(coords, values = [0, 0, 0, 0]) {
		// group by column
		const deletionsByColumn = coords.reduce(
			(deletions, coord) => {
		...
		deletionsByColumn.forEach((deletions, x) => {
			...
			const additions = deletions.map((coord, depth) => {
				...
				const outAnimation = Animate.explode(this.getTile(coord), {
					duration: 200,
				});
	...
	getBorderingPositions(pos, exploredPositions = []) {
		...
		return positions.filter(
			(pos) => !exploredPositions.find((explored) => explored.equals(pos))
		);
	}
	getEveryPossibleCoord() {
		return range(this.grid.length).reduce((rows, filler, y) => {
			return rows.concat(
				range(this.grid[y].length).map((filler, x) => {
	...
	select(tile, value) {
		if (
			Input.isMouseDown &&
			this.tilesPlayed.length < this.get("combo").length
		) {
	...
	generateValue() {
		...
		this.generationsSince.forEach(
			(count, index) => index != value && this.generationsSince[index]++
		);
		return value;
	}
	submit() {
		...
		if (
			combo.length == this.comboPlayed.length &&
			(serializeData(combo) == serializeData(this.comboPlayed) ||
				serializeData(reverseCombo) == serializeData(this.comboPlayed))
		) {
			this.replaceTiles(
				this.tilesPlayed,
				range(combo.length).map(() => this.generateValue())
			);
			this.trigger("submit");
		} else {
	...
	render(ctx) {
		...
		this.tilesPlayed.forEach((tile, index, tiles) => {
			...
			if (index == tiles.length - 1) {
				...
				drawLine(
					getCenterOfTile(tile),
					Input.mousePos.diff(this.getGlobalPosition())
				);
			}
		});
...
class Level extends GameObject {
	constructor({ comboLength, name = ``, board = ``, time = 0, ...options }) {
	...
	gameOver() {
		...
		this.getChild("board").children.forEach((child, index) => {
			Animate.slide(child, {
				duration: 400,
				delay: (3 - Math.floor(index / 4)) * 50,
				to: child.pos.add(new Vector(0, 30)),
			});
		});
		...
		Game.root
			.getChild("main")
			.addChild(new ResultsScreen({ score: this.score, level: this }));
	}
	generateCombination() {
		if (Math.random() > 2 / 3) {
			return range(this.comboLength).map(() => Math.floor(Math.random() * 4));
		} else {
			return this.getChild("board").coordsToValue(
				this.getChild("board").getRandomCoords(this.comboLength)
			);
		}
	}
	nextTurn() {
...
class Score extends GameObject {
	...
	increment() {
		const popup = new GameText({
			...
			pos: new Vector(
				Math.round(
					(Game.viewRes.x - 16) / 2 + this.labelObject.getTextMetrics().x / 2
				),
				0
			),
		});
		...
		super({
			levels: [
				...
				},
				{
					...
					unlockCondition: "SCORE AT LEAST 20 POINTS IN '3 IN A ROW' TO UNLOCK",
					board: "55 55 55",
					time: 60000,
				},
				{
					...
					unlockCondition: "SCORE AT LEAST 15 POINTS IN 'GET SQUARE' TO UNLOCK",
					board: "f1 78 1a",
					time: 60000,
				},
				{
					...
					unlockCondition: "SCORE AT LEAST 30 POINTS IN ARCADE TO UNLOCK",
					board: "eb 69 28",
					time: 60000,
				},
			],
						...
						(level, index) =>
							new Button({
								...
								onMount: function () {
									if (
										!this.locked &&
										!window.localStorage.getItem(`unlocked,${level.name}`)
									) {
...
class Menu extends GameObject {
	createChildren() {
		...
		return [
			...
			new Flexbox({
				id: "body",
				pos: new Vector(
					offset.add(padding).x,
					offset.add(padding.mul(3)).add(title.size).y
				),
				size: flexSize,
				align: "center",
				justify: "start",
				...
				children: [
					...
					new Button({
						...
						onMount: function () {
							if (
								!this.locked &&
								!window.localStorage.getItem("unlocked,ARCADE")
							) {
								const lock = this.createLock();
								this.addChild(lock);
								this.opacity = 0.5;
	...
	playArcade() {
		if (TrophyCase.getTrophy("HIGH FIVE").getBest() >= 10) {
			...
			Game.root.getChild("main").addChild(
				new Modal({
					text: "SCORE AT LEAST 10 POINTS IN 'HIGH FIVE' TO UNLOCK",
				})
			);
		}
	}
...
class Title extends GameObject {
	constructor({ size = new Vector(45, 17), animate = false, ...options } = {}) {
	...
	createChildren() {
		...
		const tweenedMovement = function () {
			Animate.slide(this, {
				delay,
				duration,
				to: this.pos.diff(new Vector(slideAmount)),
			});
		};
			...
			new GameText({
				...
				onMount: function () {
					Animate.slide(this, {
						delay,
						duration,
						to: this.pos.add(new Vector(47 - slideAmount)),
					});
				},
			}),
			new Rectangle({
			...
			new GameText({
				...
				onMount: function () {
					Animate.slide(this, {
						delay,
						duration,
						to: this.pos.diff(new Vector(slideAmount + 6)),
					});
				},
			}),
			new Rectangle({
		...
		return [
			...
			new Flexbox({
				...
				onMount: function () {
					...
					const lifeSpan = 600;
					const animation = Animate.fadeIn(this, {
						duration: initialChildrenCount * lifeSpan,
						delay: 2500,
					});
					animation.on("start", () => {
						this.addChildren(
									...
									new Tile({
										...
										onMount: function () {
											...
											interval.on("tick", () => {
												...
												interval.interval =
													life.progress / initialChildrenCount / 4;
					...
					animation.on("end", async () => {
						...
						const ogPos = title.pos.clone();
						const center = Game.viewRes
							.mul(1 / 2)
							.diff(title.getGlobalPosition())
							.diff(title.size.mul(1 / 2).round());
						const startPos = new Vector(ogPos.x, center.y);
						title.pos = startPos;
						await Animate.slide(title, {
							duration: 800,
							delay: 800 + 500,
							to: ogPos,
						}).promise;
